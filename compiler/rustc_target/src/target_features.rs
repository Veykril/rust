...
#[derive(Debug, Clone)]
pub enum Stability<Toggleability, S> {
...
pub type StabilityUncomputed<S> = Stability<AllowToggleUncomputed, S>;
/// `Stability` where `allow_toggle` has already been computed.
pub type StabilityComputed<S> = Stability<AllowToggleComputed, S>;

#[cfg(feature = "nightly")]
impl<CTX, Toggleability: HashStable<CTX>> HashStable<CTX> for Stability<Toggleability, Symbol> {
    #[inline]
...
impl<Toggleability, S> Stability<Toggleability, S> {
    ...
    pub fn requires_nightly(&self) -> Option<S>
    where
        S: Copy,
    {
        match *self {
...
impl<S: Copy> StabilityUncomputed<S> {
    pub fn compute_toggleability(&self, target: &Target) -> StabilityComputed<S> {
        use Stability::*;
...
#[cfg(feature = "nightly")]
pub use self::list::all_rust_targets;
#[cfg(feature = "nightly")]
mod list {
    use rustc_hash::{FxHashMap, FxHashSet};

    use super::{Stability, StabilityUncomputed};
    use crate::spec::Target;

    // Constructors for the list below, defaulting to "always allow toggle".
    const STABLE: StabilityUncomputed<Symbol> =
        Stability::Stable { allow_toggle: |_target, _enable| Ok(()) };
    const fn unstable(nightly_feature: Symbol) -> StabilityUncomputed<Symbol> {
    ...
    const AARCH64_FEATURES: &[(&str, StabilityUncomputed, ImpliedFeatures)] = &[
        ...
        // FEAT_SME_B16B16
        ("sme-b16b16", unstable(sym::aarch64_unstable_target_feature), &[
            "bf16",
            "sme2",
            "sve-b16b16",
        ]),
    ...
    const AARCH64_FEATURES_FOR_CORRECT_VECTOR_ABI: &'static [(u64, &'static str)] =
        &[(128, "neon")];

    // We might want to add "helium" too.
    const ARM_FEATURES_FOR_CORRECT_VECTOR_ABI: &'static [(u64, &'static str)] = &[(128, "neon")];

    const POWERPC_FEATURES_FOR_CORRECT_VECTOR_ABI: &'static [(u64, &'static str)] =
        &[(128, "altivec")];
    const WASM_FEATURES_FOR_CORRECT_VECTOR_ABI: &'static [(u64, &'static str)] =
        &[(128, "simd128")];
    const S390X_FEATURES_FOR_CORRECT_VECTOR_ABI: &'static [(u64, &'static str)] =
        &[(128, "vector")];
